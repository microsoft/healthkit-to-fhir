//
//  WalkingHeartRateAverageConversionTests.swift
...
class WalkingHeartRateAverageConversionSpec : QuickSpec {
    override func spec() {
        describe("a walking heart rate average sample conversion") {
            context("the output FHIR Observation") {
                let expectedDate = Date.init(timeIntervalSince1970: 0)
                
                let sample = HKQuantitySample.init(type: HKQuantityType.quantityType(forIdentifier: .walkingHeartRateAverage)!, quantity: HKQuantity(unit: HKUnit(from: "count/min"), doubleValue: 90), start: expectedDate, end: expectedDate)
                
                let expectedCoding = Coding()
                expectedCoding.code = FHIRString("HKQuantityTypeIdentifierWalkingHeartRateAverage")
                expectedCoding.display = FHIRString("Walking Heart Rate Average")
