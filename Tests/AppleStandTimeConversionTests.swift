//
//  AppleStandTimeConversionTests.swift
...
class AppleStandTimeConversionSpec : QuickSpec {
    override func spec() {
        describe("an apple stand time sample conversion") {
            context("the output FHIR Observation") {
                let expectedDate = Date.init(timeIntervalSince1970: 0)
                
                let sample = HKQuantitySample.init(type: HKQuantityType.quantityType(forIdentifier: .appleStandTime)!, quantity: HKQuantity(unit: HKUnit(from: "ms"), doubleValue: 60000), start: expectedDate, end: expectedDate)
                
                let expectedCoding = Coding()
                expectedCoding.code = FHIRString("HKQuantityTypeIdentifierAppleStandTime")
                expectedCoding.display = FHIRString("Apple Stand Time")
