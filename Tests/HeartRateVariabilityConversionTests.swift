//
//  HeartRateVariabilityConversionTests.swift
...
class HeartRateVariabilityConversionSpec : QuickSpec {
    override func spec() {
        describe("a heart rate variability sample conversion") {
            context("the output FHIR Observation") {
                let expectedDate = Date.init(timeIntervalSince1970: 0)
                
                let sample = HKQuantitySample.init(type: HKQuantityType.quantityType(forIdentifier: .heartRateVariabilitySDNN)!, quantity: HKQuantity(unit: HKUnit(from: "ms"), doubleValue: 22), start: expectedDate, end: expectedDate)
                
                let expectedCoding = Coding()
                expectedCoding.code = FHIRString("80404-7")
                expectedCoding.display = FHIRString("R-R interval.standard deviation (Heart rate variability)")
