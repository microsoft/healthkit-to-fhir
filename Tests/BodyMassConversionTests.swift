    ...
    override func spec() {
        describe("a body mass sample conversion") {
            context("the output FHIR Observation") {
                ...
                let sample = HKQuantitySample.init(type: HKQuantityType.quantityType(forIdentifier: .bodyMass)!, quantity: HKQuantity(unit: HKUnit(from: "lb"), doubleValue: 185), start: expectedDate, end: expectedDate)
                ...
                context("when using the default config") {
                    ...
                    it("includes the expected value") {
                        if let value = observation.valueQuantity{
                            expect(value.value) == 185
                            expect(value.code) == "[lb_av]"
                            expect(value.unit) == "lb"
